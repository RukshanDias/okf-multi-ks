...
class ReferenceRunner:
    def __init__(
        ...
        self.web_extra_instruction = web_extra_instruction
    ...
    def run_web_pass(self) -> None:
        ...
        try:
            ...
            message = _build_web_user_message(
                ...
                allowed_path_prefixes=self.web_allowed_path_prefixes,
                denied_path_substrings=self.web_denied_path_substrings,
                extra_instruction=self.web_extra_instruction,
            )
    ...
    def run_web_only(self) -> None:
        """Run the web-ingestion pass and regenerate indexes (no BQ pass)."""
        self.run_web_pass()
        ...
        *,
        ...
        if web_only:
            if not self.web_seeds:
                raise ValueError("web-only mode requires at least one --web-seed URL")
            self.run_web_only()
            return 0
