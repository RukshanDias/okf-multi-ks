...
def _parser() -> argparse.ArgumentParser:
    ...
    enrich.add_argument(
        "--web-only",
        action="store_true",
        help="Skip the primary source pass and run only the web-ingestion pass.",
    )
    enrich.add_argument(
        "--web-instruction",
        default="",
        help="Extra instruction appended to the web-ingestion agent prompt.",
    )
    enrich.add_argument(
        "--no-web",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "enrich":
        ...
        runner = ReferenceRunner(
            ...
            web_extra_instruction=args.web_instruction,
            verbose=args.verbose,
        )
        only = (
            [parse_concept_id(c) for c in args.concept] if args.concept else None
        )
        if args.web_only and args.no_web:
            raise SystemExit("--web-only and --no-web cannot be used together")
        n = runner.enrich_all(only=only, web_only=args.web_only)
        if args.web_only:
            web_note = f"; web-only pass used {len(seeds)} seed(s)"
        else:
